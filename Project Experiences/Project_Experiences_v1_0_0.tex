...
\todo{Innan vi skickar in bör allt läsas igenom, och kolla så att hela rapporten är skriven i dåtid, försök ändra så fort ni ser nått.. /Knorn}
...
For this release we also created a data model\todo{vad för data model, reqT? Den gjordes till r1.. /Knorn} and some improvements to our previous context diagram which can be found in the SRS \cite{srs}.\todo{Kanske bättre att nämna SRSen först i dokumentet, så vi slipper länka den hela tiden, blir lite konsigt när vi bara gör det i sltuet av styckena också.. 

Tycker detta är ok/JOKL

När jag väl tänker efter ska väl referenser bara va första ggn de nämns i ett dokument, referensen i slutet på 1.1 räcker isf /Knorn}
...
Ser det ok ut nu? /Badju

ser ok ut, har ändrat alla förkortningar igen /David}
...
\newline 
\indent
There is no way to prevent the customer from changing their mind and this is one of the things that makes the life of a software requirements engineer so varying since you never know what ideas the customer might come up with. However if we in our project would have baselined the SRS, \todo{Har vi gjort det här nu i vid det här lager eller är det rent teoretiskt? /Knorn

-Rent teoretiskt.. Har ni bättre saker vi gjort för att unvika att kunden ändrat sig så fyll på bara /JOKL

-Formulerade om så att det framgår att det är teoretiskt, känns inte som att egnelskan är den bästa (tror det kan va lite krångel med tempus-form och så bla), nån får gärna kolla över den med färska ögon.. /Knorn

}we could tell the customer to make a change request and then an impact analysis could be carried out. This might discourage the customer to change their mind since they have to accept the additional costs found in the impact analysis.
...
Förstår inte riktigt den här TODO:n eller problemet den belyser, texten är väl bara ett exempel på hur ett scenario kompletterar krav som kan tolkas på olika sätt? /Badju

Ingen TODO per se, i brist på kommentars-möjligheter i sharelatex la jag in det såhär.. Jag ändrade texten för att det stod att vi hade lagt till det som ett scenario med krav, men det är inte så vi använder scenarios (lucid scenarios eller nått sånt skit kallas det, Nick har koll), så jag formulerade om det så att det beskriver hur vi använder det, ville bara att ni skulle kolla så att jag beskrev det på rätt sätt.. /Knorn} and brainstormed how such a scenario might play out.
...
\todo{Tar mycket plats i dolumentet, kanske bara ska referera till SRSen eftersom detta ändå gäller R3, i de andra fallen är det inte så att de "följer med", alternativt lägga i appendix?}
\begin{itemize}
\item Goals
\item General
\item Safety
...
\todo{VAD BETYDER DET HÄR STYCKET!? Säger ju totalt emot stycket innan, vilket är rätt? Kanske kan fixas genom att skrivas om så att man förstår varför det gick fel, vems fel det var och vad som hade kunnat göras bättre? /knorn}
\todo{Det här är väl ok? /JOKL}
\todo{Jag var inte aktiv i det här arbetet, så jag vet inte. Den som kommenterade under stycket kan nog ge bättre input /Knorn}
...
\todo{Jag tycker det här är lite väl att ta i, att vi frågade om deliverables fanns med var ju ett misstag, har inget att göra med om vi är på rätt spår eller inte, men det är bra med lite reflektioner över detta hursomhelst. /JOKL}
...
\todo{Knorn hade synpunkter på det sista stycket men vi behöver någon form av reflektion här hur som helst! /JOKL}
...
When we had finished the checklist we still had some comments about missing and ambiguous requirements. These where added to the checklist which we sent to the developers. We also decided that having a validation meeting with the developers would be necessary in order to make sure they understood our concerns.\todo{Ska vi verkligen nämna både i inledningen och i det här stycket att vi hade ett v-meeting? /Knorn}
\todo{Jag tycker att detta är ok! //JOKL}

\subsection{Validation meeting}
During the validation process we found more comments than could fit into their validation checklist so we decided to setup a meeting\todo{Nu nämner vi en tredje gång att vi har ett möte!? Det här stycket skulle kunna börja vid "During this bla bla, om det formulerades om.. /Knorn}
\todo{Jag tycker detta är helt ok! //JOKL}
...
\todo{Få gärna med en referens till bilden på mindmappen i appendix B}
...
\todo{Fram till hit ar samma som sags i SRS, vet ej vad som ska vara var?/JOKL}
